...
* @zh
* 使用Hertel-Mehlhorn算法将输入的凹多边形拆分成多个凸多边形。如果输入多边形是凸多边形，将返回原多边形。
* @param polygon @en one polygon. @zh 一个多边形。
* @return @en polygon array. @zh 多边形数组。
*/
export function ConvexPartition (polygon: Vec2[]) {
    ...
    for (; triangles.length;) {
        ...
        for (let iPoly = 0, polyLen = poly.length; iPoly < polyLen; ++iPoly) {
            ...
            for (; iTri2 < triangles.length; ++iTri2) {
                const triangle = triangles[iTri2] as [Vec2, Vec2, Vec2];
                for (let i = 0; i < 3; ++i) {
...
function equals (a: Vec2, b: Vec2) {
    return a.x === b.x && a.y === b.y;
}

function isClockwise (polygon: Vec2[]) {
    let sum = 0;
    for (let i = 0, len = polygon.length; i < len; ++i) {
        const p1 = polygon[i];
...
function ForceCounterClockWise (vertices: Vec2[]) {
...
function Triangulate (polygon: Vec2[]) {
    ForceCounterClockWise(polygon);
