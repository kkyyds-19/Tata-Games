import { _decorator, Component, Node, Sprite, Label, SpriteAtlas, resources, Prefab, instantiate } from 'cc';
...
//item 商品项 和 金币  通用
@ccclass('CoinShopItem_pk')
export class CoinShopItem_pk extends Component {
  @property(Sprite)
    ...
    onEnable() {
        try {
            if (this.node && (this.node as any).isValid !== false) {
                this.node.off(Node.EventType.TOUCH_END, this.onBuyButtonClicked, this);
                this.node.on(Node.EventType.TOUCH_END, this.onBuyButtonClicked, this);
            }
        } catch {}
    }
