...
export class Invite extends Component {
    ...
    private async populateSkinBlocks() {
        ...
        for (let i = 0; i < displayItems.length; i++) {
            ...
            const itemComponentPk = blockNode.getComponent(PkItem);
            const itemComponentInvite = blockNode.getComponent(InviteItem);
            const itemComponent: any = itemComponentPk || itemComponentInvite;
            if (itemComponent) {
                ...
                console.warn(`[PkMain] 列表项组件获取失败（既无 PkItem 也无 InviteItem），排序位=${i}`);
            }
        }
