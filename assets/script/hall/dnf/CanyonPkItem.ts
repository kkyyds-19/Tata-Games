...
@ccclass('CanyonPkItem')
export class CanyonPkItem extends Component {
    ...
    @property({ type: Node })
    public instantWinButton: Node = null;
    ...
    private bindInstantWinButton() {
        if (this.instantWinButton) {
            this.instantWinButton.off(Node.EventType.TOUCH_START, this.onInstantWinStart, this);
            this.instantWinButton.off(Node.EventType.TOUCH_END, this.onInstantWinClick, this);
            this.instantWinButton.on(Node.EventType.TOUCH_START, this.onInstantWinStart, this);
            this.instantWinButton.on(Node.EventType.TOUCH_END, this.onInstantWinClick, this);
            return;
        }
        const stack: Node[] = [this.node];
        while (stack.length) {
            const n = stack.pop();
            const nn = (n.name || '').toLowerCase();
            if (nn.includes('win') || nn.includes('victory') || nn.includes('instant') || nn.includes('一键') || nn.includes('胜利')) {
                n.off(Node.EventType.TOUCH_START, this.onInstantWinStart, this);
                n.off(Node.EventType.TOUCH_END, this.onInstantWinClick, this);
                n.on(Node.EventType.TOUCH_START, this.onInstantWinStart, this);
                n.on(Node.EventType.TOUCH_END, this.onInstantWinClick, this);
                this.instantWinButton = n;
                return;
            }
            n.children.forEach(ch => stack.push(ch));
        }
    ...
    private onInstantWinStart(event: any) {
        try { event?.stopPropagation && event.stopPropagation(); } catch {}
    }

    private async onInstantWinClick(event: any) {
        try { event?.stopPropagation && event.stopPropagation(); } catch {}
        await this.applyChallengeVictory();
    }
    ...
    onEnable() {
        try { director.on(game.gameEvent.HALL_USER_INFO_UPDATE, this.refreshRankFromGulchInfo, this); } catch {}
    }

    onDisable() {
        try { director.off(game.gameEvent.HALL_USER_INFO_UPDATE, this.refreshRankFromGulchInfo, this); } catch {}
    }
    ...
    private parseReward(raw: any): { canyonCrystal: number, gold: number } {
        const d = typeof raw === 'string' ? JSON.parse(raw) : (raw || {});
        const cc = Number(d?.canyon_crystal) || 0;
        const gold = Number(d?.currency_gold) || 0;
        return { canyonCrystal: cc, gold };
    }

    private applyRewardValues(cc: number, gold: number): void {
        const ui = UserInfoData.getInstance();
        if (gold > 0) ui.addGold(gold);
        if (cc > 0) ui.addHonor(cc);
        try { director.emit(game.gameEvent.HALL_USER_INFO_UPDATE); } catch {}
    }

    public applyRewardFromResponse(resp: any): void {
        if (!resp) return;
        const raw = resp.data ?? resp;
        const r = this.parseReward(raw);
        this.applyRewardValues(r.canyonCrystal, r.gold);
    }

    public async applyChallengeVictory(): Promise<void> {
        let opponentUserId: any = (game.myGlobal as any)?.arenaOpponentUserId;
        if (!opponentUserId) opponentUserId = this.getOpponentUserId();
        const uid = Number(opponentUserId);
        if (!uid || !isFinite(uid)) return;
        try {
            const resp = await userAPI.gulchChallenge({ challengeUserId: uid, challengeResult: 1 });
            const raw = resp && resp.data ? resp.data : null;
            const r = this.parseReward(raw);
            this.applyRewardValues(r.canyonCrystal, r.gold);
            try { director.emit(game.gameEvent.HALL_REWARD_POPUP_SHOW, { canyonCrystal: r.canyonCrystal, currencyGold: r.gold }); } catch {}
            try { const f = (game as any).myGlobal?.gulchChallenge; if (!f) this.decrementDailyChallengeAndEmit(); } catch {}
