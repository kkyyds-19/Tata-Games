import { _decorator, Component, Node, Label, Prefab, instantiate, sys, Button, director, game, Sprite, resources, UITransform, view, UIOpacity, Color, BlockInputEvents } from 'cc';


import { UserInfoData } from '../../user/UserInfoData';
import { Hall } from '../hall';
import { PkItem } from './PkItem';
import { CanyonPkItem } from './CanyonPkItem';
...
@ccclass('CanyonPkMain')
export class CanyonPkMain extends Component {
    ...
    onEnable() {
        ...
        this.ensureInviteBackdrop();
        this.ensureContentConsumesInput();
        this.bindOpenButtonProperty();
    }

    onDisable() {
        // 取消事件监听，避免无效回调和内存泄露
        director.off(game.gameEvent.HALL_USER_INFO_UPDATE, this.updateCurrencyDisplay, this);
        this.unbindOpenButtonProperty();
    }
    ...
    private async populateSkinBlocks() {
        ...
        const items: Row[] = [];
         ...
         try {
            ...
            const resp = await rankingAPI.getGulchCharts(this.pageNum, this.pageSize);
            if (resp && resp.code === 200) {
                ...
                if (resp && resp.data) {
                    const tp = Number((resp.data as any).totalPage) || this.totalPages;
                    const pg = Number((resp.data as any).page) || this.pageNum;
                    const sz = Number((resp.data as any).size) || this.pageSize;
                    if (tp > 0) this.totalPages = tp;
                    if (pg > 0) this.pageNum = pg;
                    this.pageSize = 5;
                }
                // 将返回结构映射为 RankingInfo：fightPower 使用原始战斗力；峡谷晶石作为 Row.honor
                const rankingData: RankingInfo[] = (honorArray || []).map((it: any) => {
                 ...
                 if (rankingData.length > 0) {
                     for (let i = 0; i < rankingData.length; i++) {
                        ...
                        const honor = (typeof honorSrc?.production === 'number') ? honorSrc.production : (typeof honorSrc?.canyonCrystal === 'number') ? honorSrc.canyonCrystal : (typeof honorSrc?.integral === 'number') ? honorSrc.integral : 0;
                         items.push({
                             type: 'pk',
        ...
        const displayItems: Row[] = rankedItems.slice(0, this.pageSize).sort((a, b) => (a.rank || 0) - (b.rank || 0));

        for (let i = 0; i < displayItems.length; i++) {
            const it = displayItems[i];
            const blockNode = instantiate(this.pkItemPrefab);
            blockNode.setPosition(385, -i * 300, 0);
            const itemComponentPk = blockNode.getComponent(PkItem) || (blockNode.getComponentsInChildren(PkItem)[0] || null);
            const itemComponentInvite = blockNode.getComponent(InviteItem) || (blockNode.getComponentsInChildren(InviteItem)[0] || null);
            let itemComponentCanyon = blockNode.getComponent(CanyonPkItem) || (blockNode.getComponentsInChildren(CanyonPkItem)[0] || null);
            let itemComponent: any = itemComponentPk || itemComponentInvite || itemComponentCanyon;
            if (!itemComponent) {
                itemComponentCanyon = blockNode.addComponent(CanyonPkItem);
                itemComponent = itemComponentCanyon;
            }
            if (itemComponent) {
                // 无论来源是 pk 还是 pk1，统一注入战力与昵称
                itemComponent.init(it.sourceIdx, it.rarity, this.hallInstance, it.power, it.name, (sender)=>{
                    console.log("[InviteMain] PkItem点击回调", sender);
                }, this);
                // 注入荣誉积分（与战力分离显示）
                ...
                if (it.type === 'pk1') {
                    try {
                        ...
                        const pk1Node = new Node('PkItem1Embed');
                        ...
                        if (pk1Comp) {
                            ...
                            try {
                                ...
                                if (pk1Comp.honorLabel) {
                                    pk1Comp.honorLabel.string = `+${hv}/h`;
                ...
                console.warn(`[PkMain] 列表项组件获取失败（既无 PkItem 也无 InviteItem 也无 CanyonPkItem），排序位=${i}`);
            }
        }
    ...
    public hide() {
        ...
        this.removeInviteBackdrop();
    }

    private ensureInviteBackdrop() {
        const parent = this.node.parent || director.getScene()?.getChildByName('Canvas') || director.getScene();
        if (!parent) return;
        let mask = parent.getChildByName('Invite_main_Mask');
        if (!mask) {
            mask = new Node('Invite_main_Mask');
            const ui = mask.addComponent(UITransform);
            const size = view.getVisibleSize();
            ui.setContentSize(size);
            mask.setPosition(0, 0, 0);
        ...
        try {
            ...
            mask.setSiblingIndex(Math.max(0, idx));
            this.node.setSiblingIndex(mask.getSiblingIndex() + 1);
        } catch {}
        mask.active = true;
        mask.off(Node.EventType.TOUCH_START, this.closeInvite, this);
        mask.off(Node.EventType.TOUCH_END, this.closeInvite, this);
        (mask as any).once?.(Node.EventType.TOUCH_START, this.closeInvite, this) || mask.on(Node.EventType.TOUCH_START, this.closeInvite, this);
    }
    ...
    private removeInviteBackdrop() {
        const parent = this.node.parent || director.getScene()?.getChildByName('Canvas') || director.getScene();
        if (!parent) return;
        const mask = parent.getChildByName('Invite_main_Mask');
        try {
            if (mask && (mask as any).isValid !== false) {
                mask.removeFromParent();
                mask.destroy();
    ...
    private ensureContentConsumesInput() {
        try {
            if (!this.node.getComponent(BlockInputEvents)) {
                this.node.addComponent(BlockInputEvents);
            }
            const handler = (event: any) => { try { event?.stopPropagation && event.stopPropagation(); } catch {} };
            const bindOn = (n: Node) => {
                n.off(Node.EventType.TOUCH_START, handler, this);
                n.off(Node.EventType.TOUCH_END, handler, this);
                n.on(Node.EventType.TOUCH_START, handler, this);
            ...
            while (queue.length) {
                const curr = queue.shift()!;
                const ui = curr.getComponent(UITransform);
                const name = (curr.name || '').toLowerCase();
                if (ui && (name.includes('panel') || name.includes('content') || name.includes('bg') || name.includes('background') || name.includes('container') || name.includes('scroll'))) {
                    bindOn(curr);
    ...
    private bindOpenButtonProperty() {
        if (!this.openButton) return;
        this.openButton.off(Node.EventType.TOUCH_END, this._onOpenByProperty, this);
        this.openButton.on(Node.EventType.TOUCH_END, this._onOpenByProperty, this);
    }

    private unbindOpenButtonProperty() {
        if (!this.openButton) return;
        this.openButton.off(Node.EventType.TOUCH_END, this._onOpenByProperty, this);
    }

    private _onOpenByProperty() {
        CanyonPkMain.openOnCanvas();
    }

    public static openOnCanvas(): void {
        const scene = director.getScene();
        const parent = scene?.getChildByName('Canvas') || scene;
        if (!parent) return;
        const exist = parent.getChildByName('Canyon_pk_main_Instance');
        if (exist) {
            exist.active = true;
            exist.setSiblingIndex(9999);
            const comp = exist.getComponent(CanyonPkMain);
            if (comp) comp.show();
            return;
        }
        resources.load('prefab/hall/dnf/Canyon_pk_main', Prefab, (err, prefab) => {
            if (err || !prefab) return;
            const n = instantiate(prefab);
            n.name = 'Canyon_pk_main_Instance';
            n.setPosition(0, 0, 0);
            n.setSiblingIndex(9999);
            parent.addChild(n);
            const comp = n.getComponent(CanyonPkMain);
            if (comp) comp.show();
        });
    }
 }
