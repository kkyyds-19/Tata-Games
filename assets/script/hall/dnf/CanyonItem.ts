...
@ccclass('CanyonItem')
export class CanyonItem extends Component {
    ...
    private rankingRow: any = null;

    private static _rankingList: any[] | null = null;
    private static _rankingLoading: Promise<any[]> | null = null;

    private static async getRankingList(pageSize: number = 10): Promise<any[]> {
        if (CanyonItem._rankingList && Array.isArray(CanyonItem._rankingList)) {
            return CanyonItem._rankingList;
        }
        if (!CanyonItem._rankingLoading) {
            CanyonItem._rankingLoading = rankingAPI.getGulchCharts(1, pageSize)
                .then((resp: any) => {
                    let arr: any[] = [];
                    if (Array.isArray(resp?.data)) {
                        arr = resp.data as any[];
                    } else if (resp?.data && Array.isArray((resp.data as any).data)) {
                        arr = (resp.data as any).data as any[];
                    }
                    CanyonItem._rankingList = arr || [];
                    CanyonItem._rankingLoading = null;
                    return CanyonItem._rankingList;
                })
                .catch(() => {
                    CanyonItem._rankingLoading = null;
                    CanyonItem._rankingList = [];
                    return [];
                });
        }
        return CanyonItem._rankingLoading;
    }
    ...
    public init(idx: number, rarity: number, hallInstance: Hall, combatPower: number, nickname: string, callback: (sender: any) => void, pkMainInstance?: PkMain) {
        ...
        this.initFromRanking().catch(() => {
            this.fetchOpponentInfoAndDisplay(this.pk_idx).catch(() => {});
        });
    }
    ...
    private getOpponentUserId(): number | null {
        if (this.rankingRow && typeof this.rankingRow.userId === 'number') {
            return Number(this.rankingRow.userId);
        }
        if (typeof this.opponentId === 'number' && !isNaN(this.opponentId)) {
    ...
    private onStartClick(){
        ...
        if (remain <= 0) {
            try { director.emit(game.gameEvent.GAME_TOAST_SHOW, '今日挑战次数已用尽'); } catch {}
            return;
        }
        this.decrementDailyChallengeAndEmit();
    ...
    private async fetchOpponentInfoAndDisplay(stageId?: number): Promise<void> {
        try {
            const resp = await arenaAPI.getOpponentTeam(stageId);
            const data: any = resp && resp.data ? resp.data : null;
            if (!data) return;
            const oid = typeof data.opponentId === 'number' ? data.opponentId : null;
            this.opponentId = oid;
            if (oid) {
                game.myGlobal.arenaOpponentUserId = oid;
            }
            ...
            try {
                const r = await rankingAPI.getGulchCharts(1, 50);
                const list: any[] = (r && r.data && (r.data as any).data) ? (r.data as any).data : [];
                if (oid && Array.isArray(list) && list.length > 0) {
                    const found = list.find((it: any) => Number(it?.userId) === oid);
                    if (found && typeof found.canyonCrystal === 'number') {
                        this.setHonorValue(found.canyonCrystal);
                    }
                }
            } catch {}
        } catch {}
    ...
    private getItemIndex(): number {
        ...
        if (typeof this.pk_idx === 'number' && this.pk_idx >= 0) return this.pk_idx;
        if (m) {
    ...
    private decrementDailyChallengeAndEmit(): void {
        const kRemain = 'gulch_daily_challenge_remaining';
        let remain = this.getDailyChallengeRemaining();
        if (remain <= 0) return;
        remain -= 1;
        if (remain < 0) remain = 0;
        sys.localStorage.setItem(kRemain, String(remain));
    ...
    private async initFromRanking(): Promise<void> {
        const list = await CanyonItem.getRankingList(10);
        if (!list || list.length === 0) return;
        const idx = this.getItemIndex();
        const row = list[idx % list.length];
        if (!row) return;
        this.rankingRow = row;
        const nickname = (row.nickName && row.nickName.length > 0) ? row.nickName : (row.userName || '未知玩家');
        const power = typeof row.fightPower === 'number' ? row.fightPower : 0;
        const honor = (typeof (row as any).production === 'number') ? (row as any).production : (typeof (row as any).canyonCrystal === 'number') ? (row as any).canyonCrystal : (typeof (row as any).integral === 'number') ? (row as any).integral : 0;
        const uid = typeof row.userId === 'number' ? row.userId : null;
        if (uid) this.opponentId = uid;
        this.nicknameValue = nickname || this.nicknameValue;
        this.combatPowerValue = power;
        this.setHonorValue(honor);
        this.updateName(0);
        this.updateCombatPowerDisplay();
        this.updateRankLabel(idx + 1);
    }
    ...
    onEnable() {
        try { director.on(game.gameEvent.HALL_USER_INFO_UPDATE, this.refreshAfterChallenge, this); } catch {}
    }

    onDisable() {
        try { director.off(game.gameEvent.HALL_USER_INFO_UPDATE, this.refreshAfterChallenge, this); } catch {}
    }

    private refreshAfterChallenge(): void {
        CanyonItem._rankingList = null;
        CanyonItem._rankingLoading = null;
        this.initFromRanking().catch(() => {
            this.fetchOpponentInfoAndDisplay(this.pk_idx).catch(() => {});
        });
