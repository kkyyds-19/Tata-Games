...
export class InviteItem extends Component {
    ...
    private onInviteClick(event: any) {
        ...
        try {
            ...
            if (opponentUserId) {
                (game.myGlobal as any).dungeonOpponentUserId = opponentUserId;
                const invite = this.resolveInviteMainInstance();
                if (invite && typeof (invite as any).hide === 'function') {
                    try { (invite as any).hide(); } catch {}
                }
            }
        } catch {}
    }
    ...
    private resolveInviteMainInstance(): InviteMain | null {
        const viaHall = this.hallInstance && (this.hallInstance as any).inviteMain;
        if (viaHall) return viaHall as InviteMain;
        try {
            const scene = director.getScene();
            if (!scene) return null;
            const stack: Node[] = [scene];
            while (stack.length) {
                const n = stack.pop()!;
                const comp = n.getComponent(InviteMain as any);
                if (comp) return comp as InviteMain;
                const children = n.children || [];
                for (let i = 0; i < children.length; i++) stack.push(children[i]);
            }
        } catch {}
